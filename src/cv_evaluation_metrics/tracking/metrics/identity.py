...
class IDENTITY(TBaseMetric):
    """Class which implements ID metrics"""

    def __init__(self) -> None:
        super().__init__()
        self.metric_list = ['IDF1', 'IDRecall', 'IDPrecision']
        self.metric_supp_list = ['IDTP', 'IDFN', 'IDFP']
    ...
    def run_eval(self, cfg: TMetricConfig):
        ...
        for timestamp, (gt_ids_t, pred_ids_t) in enumerate(zip(cfg.gt_ids, cfg.pred_ids)):
            ...
            matches_mask = np.greater_equal(cfg.similarity_scores[timestamp], cfg.threshold)
