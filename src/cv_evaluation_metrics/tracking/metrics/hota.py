...
class HOTA(TBaseMetric):
    """Class which implements the HOTA metrics"""

    def __init__(self) -> None:
        super().__init__()
        self.array_labels = np.arange(0.05, 0.99, 0.05)
        self.metric_supp_list = ['HOTA_TP', 'HOTA_FN', 'HOTA_FP']
        self.metric_list = ['HOTA', 'DetA', 'AssA', 'DetRe', 'DetPr', 'AssRe', 'AssPr', 'LocA', 'RHOTA']

    def run_eval(self, cfg: TMetricConfig):
        metric_dict = {}
        for field in self.metric_list + self.metric_supp_list:
            metric_dict[field] = np.zeros((len(self.array_labels)), dtype=np.float)

        # Return result quickly if pred_ids_t or gt sequence is empty
        if cfg.pred_dets_cnt == 0:
            metric_dict['HOTA_FN'] = cfg.gt_dets_cnt * np.ones((len(self.array_labels)), dtype=np.float)
            metric_dict['LocA'] = np.ones((len(self.array_labels)), dtype=np.float)
            return metric_dict
        if cfg.gt_dets_cnt == 0:
            metric_dict['HOTA_FP'] = cfg.pred_dets_cnt * np.ones((len(self.array_labels)), dtype=np.float)
            metric_dict['LocA'] = np.ones((len(self.array_labels)), dtype=np.float)
            return metric_dict
    ...
    def combine_sequence(self, all_res):
        """Combines metrics across all sequences"""
        metric_dict = {}
        for field in self.metric_supp_list:
    ...
    @staticmethod
    def _compute_final_fields(metric_dict):
